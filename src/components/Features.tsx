            ...
            <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-12 relative z-10">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 group/container">
                    {features.map((feature, idx) => (
                        <div
                            key={idx}
                            className={`p-12 rounded-sm border border-white/5 bg-[#0c0a09]/50 glass backdrop-blur-3xl hover:border-white/30 transition-all duration-700 group relative overflow-hidden hover:-translate-y-2 shadow-2xl shadow-black/50`}
                        >
                            {/* Card Glow */}
                            <div className="absolute -top-12 -right-12 w-24 h-24 bg-white/5 blur-3xl rounded-full transition-all group-hover:bg-white/10"></div>

                            <div className={`mb-10 p-5 w-fit rounded-sm bg-black/50 text-zinc-400 border border-white/5 group-hover:scale-110 group-hover:border-white/30 group-hover:text-white transition-all duration-500`}>
                                {feature.icon}
                            </div>
                            <h3 className="text-2xl font-bold text-white mb-6 serif italic transition-colors group-hover:text-zinc-200">{feature.title}</h3>
