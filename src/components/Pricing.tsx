...
const Pricing: React.FC = () => {
    const tiers = [
        { name: "SOLO", price: "$299", desc: "For 1-3 attorneys", features: ["50 research queries per day", "Citation checking (unlimited)", "Case summaries (unlimited)", "Email support (24-hour response)", "Export to Word/PDF"] },
        { name: "FIRM", price: "$699", desc: "For 4-15 attorneys", features: ["Unlimited research queries", "Priority support (4-hour response)", "Team collaboration features", "Usage analytics", "Everything in Solo"] },
        { name: "ENTERPRISE", price: "Custom", desc: "For 16+ attorneys", features: ["Custom integration", "Dedicated account manager", "Custom contract terms", "Team training sessions", "Everything in Firm"] }
    ];
            ...
            <div className="max-w-[1400px] mx-auto px-4 md:px-12">
                <div className="mb-24 flex flex-col md:flex-row md:items-end justify-between gap-8">
                    <div className="max-w-2xl translate-y-4 opacity-0 animate-[reveal_1s_ease-out_forwards]">
                        <span className="font-sans text-[10px] text-zinc-400 uppercase tracking-widest mb-6 block font-bold px-4 py-1.5 border border-white/20 rounded-full w-fit">Plans</span>
                        <h2 className="text-6xl font-bold text-white serif leading-none">Transparent <br /><span className="italic gold-gradient">Pricing.</span></h2>
                    </div>
                    ...
                    {tiers.map((tier, i) => (
                        <div key={i} className={`p-16 border-r border-zinc-900 last:border-r-0 group relative transition-all duration-700 hover:z-20 flex flex-col ${i === 1 ? 'glass-strong' : 'hover:bg-zinc-900/40'}`}>
                            ...
                            <button className="mt-auto w-full py-5 border border-zinc-800 text-xs font-bold uppercase tracking-[0.2em] text-zinc-500 relative overflow-hidden transition-all duration-500 hover:border-white hover:text-black group/btn">
                                ...
                                <span className="relative z-10">{i === 0 ? "Try Free" : i === 1 ? "Get Demo" : "Contact Sales"}</span>
                            </button>
                        </div>
                    ))}
                </div>
