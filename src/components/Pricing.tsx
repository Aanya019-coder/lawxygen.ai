            ...
            <div className="max-w-[1400px] mx-auto px-4 md:px-12">
                <div className="mb-24 flex flex-col md:flex-row md:items-end justify-between gap-8">
                    <div className="max-w-2xl translate-y-4 opacity-0 animate-[reveal_1s_ease-out_forwards]">
                        <span className="font-sans text-[10px] text-zinc-500 uppercase tracking-widest mb-6 block font-bold px-4 py-1.5 border border-[#A88464]/20 rounded-full w-fit">Investment Tiers</span>
                    ...
                    <p className="text-stone-500 text-sm max-w-sm italic border-l border-[#A88464]/20 pl-6">
                    ...
                    {tiers.map((tier, i) => (
                        <div key={i} className={`p-16 border-r border-zinc-900 last:border-r-0 group relative transition-all duration-700 hover:z-20 ${i === 1 ? 'glass-strong' : 'hover:bg-zinc-950/50'}`}>
                            ...
                            <div className="mb-12 relative z-10 transition-transform duration-500 group-hover:-translate-y-2">
                                <span className="font-sans text-[10px] text-stone-600 mb-2 block uppercase tracking-tighter">{tier.desc}</span>
                                ...
                                <div className="text-2xl text-stone-400 font-light font-sans group-hover:text-white transition-colors">{tier.price} <span className="text-[10px] uppercase tracking-widest opacity-50">/ session</span></div>
                            </div>
                            ...
                            <ul className="space-y-6 mb-16 relative z-10">
                                {tier.features.map((f, j) => (
                                    <li key={j} className="flex items-center space-x-4 group/item">
                                        ...
                                        <span className="text-stone-500 text-sm transition-all duration-500 group-hover:text-stone-300 group-hover:translate-x-1">{f}</span>
                                    </li>
                                ))}
                            </ul>

                            <button className="w-full py-5 border border-zinc-800 text-xs font-bold uppercase tracking-[0.2em] text-stone-500 relative overflow-hidden transition-all duration-500 hover:border-[#A88464] hover:text-white group/btn">
